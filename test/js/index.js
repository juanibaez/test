...
$.ajax({
    ...
    success: function (result) {
        ...
        var productData = alasql("SELECT COUNT(diagcode) FROM ? order by utc",[result]);
        ...
        for (i = 0; i < 24; i++) {
            ...
            accountLicenses = alasql("SELECT COUNT(DISTINCT userid) licenses FROM ? WHERE utc >= '" + hourBefore + "' and utc < '" + hourAfter + "' order by utc",[result]);
            accountDevices = alasql("SELECT COUNT(DISTINCT userid) devices FROM ? WHERE utc >= '" + hourBefore + "' and utc < '" + hourAfter + "' order by utc",[result]);
            ...
            accountDevicesTest = alasql("SELECT utc FROM ? WHERE utc >= '" + hourBefore + "' and utc < '" + hourAfter + "' order by utc",[result]);
        ...
        function statCount() {
            ...
            var doughnutOptions = {
                legend: {display: true},
                responsive: true,
                title: {display: true, text: 'Total Licenses:' + countLicenses, verticalAlign: 'center', textAlign: 'center', textBaseline: 'middle' }
            };
...
/*************************************************************************************/
        ...
        for (i = 0; i < result.length; i++) {
            ...
            if (result[i].length > 1) {
                ...
                for (x = 0; x < result.length; x++) {
                    ...
                    if ((Date.parse(oneHoursBefore) > Date.parse(sessionTime) && (Date.parse(oneHoursAfter) < Date.parse(sessionTime)))) {
        ...
        for (i = 0; i < 100; i++) {
            ...
            getCodeData(longlatArray, userName, timeStamp, diagcode, i)
        }
        ...
        function getCodeData(longlatArray, userName, timeStamp, diagcode, count) {
            ...
            if (longlatArray[0] !== "") {
                ...
                data = {'latitude' : longlatArray[0], 'longitude' : longlatArray[1], 'localityLanguage' : 'en', 'key' : 'b0067cd12214491aa5317a26c85a007b'}
            } else {
                apiURL = 'https://api.bigdatacloud.net/data/ip-geolocation-with-confidence/';
                var ipAddress = result[i].remoteip;
                data = {'ip' : ipAddress, 'localityLanguage' : 'en', 'key' : 'd915f9ffc0134077a61239935a7673df'}
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    htmlString_1 = htmlString_1 + "<td>" + userName + "</td>";
                    /*htmlString_1 = htmlString_1 + "<td>" + address + "</td>";*/


                    htmlString_1 = htmlString_1 + "<td class='text-navy'> <i class='fa fa-level-up'></i>" + geoCodeInfo + "</td>";
                    ...
                    if (count < 4) {
                        ...
                        htmlString_2 = htmlString_2 + "<small class='float-right text-navy'>1m ago</small>";
                        htmlString_2 = htmlString_2 + "<strong>" + address + "</strong>";
                        htmlString_2 = htmlString_2 + "<div>" + geoCodeInfo + "</div>";
                        htmlString_2 = htmlString_2 + "<small class='text-muted'>" + timeStamp + "</small>";
                        htmlString_2 = htmlString_2 + "</div>";
                        htmlString_2 = htmlString_2 + "</div>";
